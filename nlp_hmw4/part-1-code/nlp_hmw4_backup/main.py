...
def create_augmented_dataloader(args, dataset):
    ...
    # Randomly sample 5,000 examples from the training set
    sampled_examples = original_train.shuffle(seed=42).select(range(5000))
    
    # Apply custom_transform to the 5,000 sampled examples
    transformed_examples = sampled_examples.map(custom_transform, load_from_cache_file=False)
    
    # Concatenate original training dataset with the 5,000 transformed examples
    augmented_dataset = concatenate_datasets([original_train, transformed_examples])
    
    # Tokenize the combined dataset
    augmented_tokenized = augmented_dataset.map(tokenize_function, batched=True, load_from_cache_file=False)
    
    # Prepare dataset for use by model (remove text, rename label to labels, set format to torch)
    ...
    # Create and return the dataloader with shuffle=True for training
