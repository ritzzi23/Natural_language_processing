...
def get_args():
    ...
    parser.add_argument('--learning_rate', type=float, default=1e-3, help="Learning rate (default: 1e-3, 1e-1 is too high and will cause divergence)")
...
def train(args, model, train_loader, dev_loader, optimizer, scheduler):
    ...
    for epoch in range(args.max_n_epochs):
        ...
        eval_loss, record_em, record_f1, sql_em, error_rate = eval_epoch(args, model, dev_loader,
...
def train_epoch(args, model, train_loader, optimizer, scheduler):
    ...
    total_loss = 0
    total_tokens = 0
    # Use ignore_index=-100 to ignore padded positions in decoder targets
    ...
    for encoder_input, encoder_mask, decoder_input, decoder_targets, _ in tqdm(train_loader):
        ...
        with torch.no_grad():
            # Count non-padded tokens (targets != -100)
            num_tokens = (targets_flat != -100).sum().item()
